...
impl MemMap32 {
    ...
    pub fn get32(&self, addr: Address, big_endian: bool) -> Option<u32> {
        ...
        match self.address_space.read_cells(addr, addr + 3) {
            ...
            Some(cells) => {
                if big_endian {
                    Some(((cells[0] as u32) << 24) + ((cells[1] as u32) << 16) +
                         ((cells[2] as u32) << 8) + (cells[3] as u32))
                } else {
                    Some(((cells[3] as u32) << 24) + ((cells[2] as u32) << 16) +
                         ((cells[1] as u32) << 8) + (cells[0] as u32))
                }
            }
        }
    }
...
#[cfg(test)]
mod test {
    use address::{Address, Cell, Addressable, Region, LeasableRegion, AddressSpace,
                  RandomAccessMemory, MemMap32};
    ...
    fn lease_first_4k_of_address_space() {
        ...
        assert!(address_space.leased_subregion_at(0xffffffffffffffff)
                             .is_none());
    ...
    fn fail_to_sublease_region_partway_out_of_bounds() {
        ...
        let ram_oob = RandomAccessMemory::new(address_space.end, address_space.end + 1);
        assert_eq!(0, address_space.leased_subregions().len());
        assert!(!address_space.available_for_lease(&ram_oob));
        assert!(address_space.lease(Box::new(ram_oob)).is_none());
        assert_eq!(0, address_space.leased_subregions().len());
        assert!(address_space.available_for_lease(&RandomAccessMemory::new(address_space.start,
                                                                           address_space.end)));
    ...
    fn build_deep_tree_of_ram_chips_and_write_to_all_cells() {
        ...
        assert!(ram256_low.lease(Box::new(RandomAccessMemory::new(0, 63)))
                          .is_some());
        assert!(ram256_low.lease(Box::new(RandomAccessMemory::new(64, 127)))
                          .is_some());
        assert!(ram256_low.lease(Box::new(RandomAccessMemory::new(128, 191)))
                          .is_some());
        assert!(ram256_low.lease(Box::new(RandomAccessMemory::new(192, 255)))
                          .is_some());

        assert!(ram256_high.lease(Box::new(RandomAccessMemory::new(256, 319)))
                           .is_some());
        assert!(ram256_high.lease(Box::new(RandomAccessMemory::new(320, 383)))
                           .is_some());
        assert!(ram256_high.lease(Box::new(RandomAccessMemory::new(384, 447)))
                           .is_some());
        assert!(ram256_high.lease(Box::new(RandomAccessMemory::new(448, 511)))
                           .is_some());
